...
const resolveFontSize = (value: any, container: HTMLElement, fontWeight: number, fontFamily: string) => {
...
const ParticleText: React.FC<ParticleTextProps> = ({
  ...
  useEffect(() => {
    ...
    const sampleText = async () => {
      ...
      let aiStartX = Number.MAX_SAFE_INTEGER;
      if (content.endsWith(' AI')) {
        const metricsBefore = offCtx.measureText(content.slice(0, -2));
        aiStartX = padding - left + metricsBefore.width;
      }
      ...
      particles = selected.map((target, index) => {
        ...
        const particleColor = target.isHighlight ? '#ffffff' : (baseRgb && highlightRgb ? rgbToCss(mixRgb(baseRgb, highlightRgb, blend)) : color);
