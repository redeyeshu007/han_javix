  ...
  useEffect(() => {
    ...
    class App {
      ...
      tick() {
        ...
        // Skip rendering when paused (off-screen)
        if (this.paused) {
    ...
    myApp.paused = true; // Start paused until visible
    appRef.current = myApp;
    myApp.loadAssets().then(myApp.init);

    // Pause/resume rendering based on visibility to save GPU/CPU
    const visibilityObserver = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (appRef.current) {
            appRef.current.paused = !entry.isIntersecting;
          }
        });
      },
